...
def update_list():
    ...
    x = int(input("which list would you like to update?- "))
    y = int(input("which item would you like to update?- "))
    nv = input(f"enter the new value for item {y} - ")
    shopping_lists[x-1][y-1] = nv

def print_list():
    global shopping_lists
    x = int(input("which shopping list would you like to print?- "))
    print(shopping_lists[x-1])

def print_item():
    global shopping_lists
    x = int(input("which list would you like to view?- "))
    y = int(input("which item would you like to view?- "))
    print(shopping_lists[x-1][y-1])

def all_in_one():
    global shopping_lists
    new_list = []
    for i in range(0, len(shopping_lists)):
        for c in range(len(shopping_lists[i])):
            new_list.append(shopping_lists[i][c])
...
def if_you_give_a_moose_a_cookie():
    ...
    for i in range(len(shopping_lists)):
        if 'milk' in shopping_lists[i]:
            shopping_lists[i].pop(shopping_lists[i].index('milk'))
            shopping_lists[i].append('milk and cookies')
        

while True:
    turn = input("choose: update, view item, or view list - ")
    if turn == 'update':
        update_list()
    elif turn == 'view item':
        print_item()
